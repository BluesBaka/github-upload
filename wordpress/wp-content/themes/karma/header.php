...
$favicon                      = $ttso->ka_favicon;

// ----- Define ALT text to be used for Logo
if( !empty( $ka_logo_alt ) ) { $truethemes_logo_text = $ka_logo_alt; } else { $truethemes_logo_text = get_bloginfo('name'); }
...
// ----- Add class for jQuery-2 or CU3ER
if( is_home() ) {
...
<div id="<?php if ( 'true' == $boxedlayout ){echo 'tt-boxed-layout';} else { echo 'tt-wide-layout';} ?>" class="<?php if ('' != $div_main_custom_color){echo 'content-custom-bg';} else {echo $div_main_style;} ?>">
	<div id="wrapper"<?php if ( 'true' == $ubermenu_styling ){echo ' class="tt-uberstyling-enabled"';} ?>>
		<header role="banner" id="header" itemscope="itemscope" itemtype="http://schema.org/WPHeader" <?php if (is_page_template('template-homepage-3D.php')){echo "style='height: 560px;'";} ?>>
...
<?php if ('true' == $ka_toolbar_left): ?>
  ...
  <?php if(has_nav_menu('Top Toolbar Navigation')):
 		...
 		echo '<ul>'; wp_nav_menu(array('theme_location' => 'Top Toolbar Navigation' , 'depth' => 0 , 'items_wrap'      => '%3$s', 'container' =>false )); echo '</ul>';
